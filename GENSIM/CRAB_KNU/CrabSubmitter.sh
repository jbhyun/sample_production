...
ProdCfgFile="skeletons/GENSIM_LFilter_cfg.py"
InputList="ListToSubmit.txt"
ReSubList="ListToReSub.txt"
KillList="ListToKill.txt"
TrigDebug="F"
RunningMode="Status" #Init/Resub/Status/Kill/Pilot
...
if [[ ${RunningMode} == "Init" || ${RunningMode} == "Pilot" ]]; then
  ...
  do
    ...
    sed -i "s|outputPrimaryDataset\ =\ '[^']*'|outputPrimaryDataset\ =\ '${Process}'|g" ${CrabCfgFull}
