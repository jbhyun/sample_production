...
if [[ ( ! -e skeletons/crab_skeleton.py ) || ( ! -e skeletons/RAWSIM_to_AODSIM.py ) ]]; then echo "One of target script doesn't exist, exiting"; exit 1; fi 
...
do
 ...
 cp skeletons/RAWSIM_to_AODSIM.py ${ProcessDir}/RAWSIM_to_AODSIM_crab_${Process}.py


 sed -i "s/##.*config\./config\./g" ${ProcessDir}/crab_${Process}.py
 sed -i "s/requestName\ =\ '[^']*'/requestName\ =\ '${Process}_CMSSW_8_0_21_AODSIM'/g" ${ProcessDir}/crab_${Process}.py
 sed -i "s/psetName\ =\ '[^']*'/psetName\ =\ 'RAWSIM_to_AODSIM_crab_${Process}.py'/g" ${ProcessDir}/crab_${Process}.py
