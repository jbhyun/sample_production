...
ProdStep="DigiPUMixL1HLT"
CrabCfgFile="skeletons/crab_skeleton.py"
ProdCfgFile="skeletons/DigiPUMixL1HLT_cfg.py"
...
if [[ ! -e ${PUFileList} ]]; then echo "PU list doesn't exist, exiting"; exit 1;
else
  PUListDir=$( readlink -f ${PUFileList} | rev | cut -d '/' -f2 | rev ) 
  PUFileName=$( readlink -f ${PUFileList} | rev | cut -d '/' -f1 | rev ) 
  if [[ ${PUListDir} != "skeletons" ]]; then
    cp -v ${PUFileList} ./skeletons/; 
    PUFileList="skeletons/${PUFileName}";
  fi
fi
...
if [[ ${RunningMode} == "Init" || ${RunningMode} == "Pilot" ]]; then
  ...
  do
    ...
    NMinbiasFile=$( wc -l ${PUFileList} | cut -d ' ' -f1 )
    for (( it_f=1; it_f<${NPUFileToMix}; it_f++ ))
    do
      MinFileLineN=$(( $RANDOM % ${NMinbiasFile} + 1 ))
      MinFile=$( sed "${MinFileLineN}q;d" ${PUFileList} )
      sed -i "/##MINBIAS_HERE##/a\ \ \ \ '${MinFile}'," ${ProdCfgFull} 
    done
