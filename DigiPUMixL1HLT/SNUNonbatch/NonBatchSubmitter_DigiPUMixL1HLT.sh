...
InputDir=""
OutDir="/data9/Users/jbhyun/PrivGEN/NoFilter/DigiPUMixL1HLT"
InFileList="FileList.txt"
OutFileNameBase="DigiPUMixL1HLT"
MinBiasList="MinbiasList_17.txt"
Script="DigiPUMixL1HLT_cfg.py"
TrigDebug="F"
TrigMVOut="T" #T or F
Nmax_phy=72
Nmax_man=50
...
do
  ...
  for (( it_f=1; it_f<10; it_f++ ))
  do
    LineNo=$(( ${FileNum} * 10 + ${it_f} ))
    MinFile=$( sed "${LineNo}q;d" ${MinBiasList} )
    sed -i "/##MINBIAS_HERE##/a\ \ \ \ 'file:${MinFile}'," ${ProcStr}/${CfgName}
  done
  if [[ ${TrigDebug} == "F" ]]; then
